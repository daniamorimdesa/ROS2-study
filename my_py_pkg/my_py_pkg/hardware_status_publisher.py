#!/usr/bin/env python3
import rclpy
from rclpy.node import Node
from my_robot_interfaces.msg import HardwareStatus

# Nó para publicar usando a interface de mensagem criada HardwareStatus

"""Este node cria um nó que publica o status do hardware do robô.

- publisher: publica mensagens de status do hardware em um tópico chamado "hardware_status"
"""

class HardwareStatusPublisherNode(Node):

    def __init__(self):
        super().__init__("hardware_status_publisher") # criar um nó chamado "hardware_status_publisher"

        # criar um publicador no tópico "hardware_status" com fila de tamanho 10
        self.hw_status_publisher_ = self.create_publisher(HardwareStatus, "hardware_status", 10) 

        # criar um timer que chama publish_hardware_status a cada 1 segundo
        self.timer_ = self.create_timer(1.0, self.publish_hardware_status) 

        self.get_logger().info("Hardware Status Publisher has been started!") 

    # função para publicar o status do hardware
    def publish_hardware_status(self):
        msg = HardwareStatus()                 # criar uma mensagem do tipo HardwareStatus
        msg.temperature = 43.7                 # definir a temperatura
        msg.are_motors_ready = True            # definir o status dos motores
        msg.debug_message = "Nothing special"  # definir a mensagem de depuração
        self.hw_status_publisher_.publish(msg) # publicar a mensagem


def main(args=None):
    rclpy.init(args=args)                    # inicializar o rclpy
    node = HardwareStatusPublisherNode()     # instanciar o nó
    rclpy.spin(node)                         # manter o nó ativo 
    rclpy.shutdown()                         # finalizar o rclpy
#-----------------------------------------------------------------------------------------------
if __name__ == "__main__":
    main()