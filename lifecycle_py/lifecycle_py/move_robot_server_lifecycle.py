...
"""
modificar move_robot_server para virar um lifecycle node
"""
#------------------------------------------------------------------------------------------------------------------------------------------
...
class MoveRobotServerNode(LifecycleNode):
    ...
    def on_configure(self, previous_state: LifecycleState):
        ...
        self.declare_parameter("robot_name", rclpy.Parameter.Type.STRING) # declarar o parâmetro para o nome do robô
        self.robot_name_ = self.get_parameter("robot_name").value         # obter o valor do parâmetro
    ...
    def on_cleanup(self, previous_state: LifecycleState): 
        ...
        self.undeclare_parameter("robot_name")                   # remover o parâmetro do nome do robô
        self.robot_name_=""                                      # limpar o nome do robô
        self.move_robot_server_.destroy()                        # destruir o Action Server
    ...
    def goal_callback(self, goal_request: MoveRobot.Goal): 
        ...
        # verificar se está o nó está ativo
        if not self.server_activated_:
            self.get_logger().info("Node not activated yet...Rejecting goal.")
        ...
        # (nova meta é válida, abortar meta atual e aceitar a nova)
        with self.goal_lock_:
            if self.goal_handle_ is not None and self.goal_handle_.is_active:
                self.get_logger().info("Aborting current goal...")
                self.goal_handle_.abort()
